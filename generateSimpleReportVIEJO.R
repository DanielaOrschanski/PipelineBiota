generateSimpleReportV <- function(patient_dir) {
  ...
  InfoGral <- info
  nombre_paciente <- InfoGral$Información[which(InfoGral$Característica == "Nombre y Apellido")]

  # Obtener datos de las tablas
  counts_tax <- counts_Tax(patients_dir = patient_dir, source = "KRAKEN")

  reportCountsTax(id=id, MetadataB = MetadataB, patients_dir = patients_dir)
  reportMasAbundantes(id = id, MetadataB = MetadataB)
  indicadorRangoEtario(id = id, MetadataB = MetadataB)

  #Armar recomendaciones: ------------------------------------------------------------------------
  out <- reportCountsTax(id = id, MetadataB = MetadataB, patients_dir = patients_dir)
...
- __%s__:%s", rec_subtitulo1, rec_texto1, rec_subtitulo2, rec_texto2)
  ...
  texto5 <- "Tu microbioma es único, como tu huella digital. En esta sección, detallaremos la variabilidad y cantidad de microorganismos en tu microbioma, destacando los grupos más abundantes. Estos índices muestran cuán equilibrados están los microorganismos en tu piel. Cuanto más cercano a 1, más equilibrado está y contribuye a mantener una piel saludable."
  titulo_score <- ""
  ...
  texto_filos <- "Los FILOS bacterianos más comunes en la piel son _Actinobacteria_,  _Firmicutes_, _Proteobacteria_ y _Bacteroidetes_."
  ...
  #}
  texto_generos <- sprintf("Los GÉNEROS más abundantes de tu piel son _%s_.", generos_top3)
  ...
  #}
  texto_especies <- sprintf("Las ESPECIES más abundantes de tu piel son _%s_.", especies_top3)

  indice_cantidad <- "5.2. Índice de cantidad"
  indice_variabilidad <- "5.1. Índice de variabilidad"


  descripcion_barplots <- "Gráfico que muestra la cantidad de Filos, Géneros y Especies más abundantes de tu muestra en comparación con el rango de referencia de la plataforma de Biotalife Skin."
  texto_abajo_especies <- "En la sección de recomendaciones se sugieren cambios en los hábitos que pueden colaborar a mejorar la cantidad y el equilibrio de los microorganismos en tu piel."
  ...
  texto_final <- "¡Gracias por ser voluntario/a en el estudio gratuito de microbioma cutáneo de Biotalife!
...
if(grepl("menor", mensaje_cantidades) | grepl("cercana", mensaje_cantidades) | grepl("menor", mensaje_score) | grepl("cercana", mensaje_score)  ) {
  mensaje_2indices <- "El análisis de tus índices indica que tu microbioma está levemente desequilibrado, con ciertas bacterias en tu piel dominando sobre otras. Esto puede aumentar la sensibilidad de tu piel y hacerla más propensa a alteraciones como inflamación, eczema, acné, dermatitis atópica, entre otras.

  En la sección de recomendaciones se sugieren cambios en los hábitos que pueden colaborar a mejorar el equilibrio de los microorganismos en tu piel."
} else {
  mensaje_2indices <- "El análisis de tus índices indica que tu microbioma se encuentra equilibrado."
}
...
knitr::asis_output(paste0("### ", indice_variabilidad, "\n\n"))
...
knitr::asis_output(paste0("### ", indice_cantidad, "\n\n"))
cat("\n\n")
...
knitr::asis_output(paste0(mensaje_2indices, "\n\n"))
cat("\n\n")

knitr::asis_output(paste0("### ", perfil_taxonomico, "\n\n"))
cat("\n\n")
...
knitr::asis_output(paste0(texto_abajo_especies, "\n\n"))
cat("\n\n")
plot(grid_plot)
cat("\n\n")
knitr::asis_output(paste0(descripcion_barplots, "\n\n"))
cat("\n\n")
...
', id, nombre_paciente, patient_dir, patient_dir, patient_dir, patient_dir, patient_dir, patient_dir, patient_dir, patient_dir,patient_dir, patient_dir, patient_dir, patient_dir, patient_dir, patient_dir, patient_dir)
  ...
  output_file <- sprintf("%s/MR_%s.pdf", patient_dir, id_mr)
  ...
  #Esto despues se va:
  file.remove(sprintf("%s/PieChart_BiomarcadoresRangoEtario.png", patient_dir))
  file.remove(sprintf("%s/fototipo_barra.png", patient_dir))
  file.remove(sprintf("%s/SimpleReportResults_Biota_ID%s.pdf", patient_dir, id))
  file.remove(sprintf("%s/SimpleReportResults_Biota_ID%s.log", patient_dir, id))
  file.remove(sprintf("%s/ReportResults_Biota_ID%s.log", patient_dir, id))
  file.remove(sprintf("%s/ReportResults_Biota_ID%s.Rmd", patient_dir, id))
  file.remove(sprintf("%s/ReportResults_Biota_ID%s.pdf", patient_dir, id))

  file.remove(sprintf("%s/QCReport_Biota.html", patient_dir))
  file.remove(sprintf("%s/QCReport_Biota.Rmd", patient_dir))
  file.remove(sprintf("%s/report.pdf", patient_dir))
  unlink(sprintf("%s/ReportResults_Biota_files", patient_dir))
  ...
  #rmarkdown::render(rmd_file, output_file = output_file, envir = new.env())
